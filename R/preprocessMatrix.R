...
preprocessMatrix <- function(x, center.cols = TRUE, scale.cols = FALSE, center.rows = FALSE,
  ...
  if(!simultaneous) {
    ...
    if(!("scaleMatrix" %in% rownames(installed.packages()))) {
      stop("scaleMatrix package required for simultaneous scaling.")
    } else {
      require(scaleMatrix)
    ...
    stop("Sorry, simultaneous row/column scaling not yet implemented.")
  }
